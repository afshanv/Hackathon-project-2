...
export default function AboutUsSection() {
  ...
  const brandFeatures = [
    { emoji: '🚍', title: 'Next Day as Standard', description: 'Order before 3 pm to get standard delivery the next day.' },
    { emoji: '💹', title: 'Fast Shipping', description: 'Enjoy fast and reliable shipping for all your orders.' },
    { emoji: '💯', title: 'Quality Assurance', description: 'We ensure the highest quality for all our products.' },
    { emoji: '👍', title: 'Customer Support', description: 'We provide excellent customer support 24/7.' },
  ];
    ...
    <section className="text-gray-600 body-font py-12">
      <div className="container px-5 md:px-28 mx-auto flex flex-wrap md:flex-nowrap justify-center md:justify-between">
        ...
        <div className="w-full md:w-1/2 px-4 md:px-8 mb-6 md:mb-0 bg-teal-600 p-8 rounded-lg shadow-lg text-white flex flex-col justify-between items-center md:items-start">
          ...
          <p className="leading-relaxed mb-6 text-center md:text-left text-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 px-6 md:px-48 py-8">
        {brandFeatures.map((item, index) => (
          <div key={index} className="bg-slate-200 p-6 md:p-8 rounded-lg shadow-md text-teal-600 flex flex-col justify-between">
            ...
            <p className="text-teal-600 text-base md:text-lg">{item.description}</p>
          </div>
        ))}
      </div>
